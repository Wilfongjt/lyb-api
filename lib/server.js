  ...
  /*
  {
    ...
    options: {
      config: {
        ...
        host: process.env.PORT || database_config.host || '<undefined-host db>',
        database: database_config.database || '<undefined-database>',
        // PORT is dynamic in heroku but must be defined in developmen
        port: process.env.PORT || database_config.port || '<undefined-port>',
        guest_token: process.env.API_TOKEN  || '<undefined-jwt-token>'
        $lab:coverage:on$ 
      }
    }
...
const strategy =  function () {
  ...
  return {
    ...
    verify: {
        /* $lab:coverage:off$ */
        aud: JSON.parse(process.env.JWT_CLAIMS).aud || '<audience>',
        iss: JSON.parse(process.env.JWT_CLAIMS).iss || '<issuer>',
        sub: JSON.parse(process.env.JWT_CLAIMS).sub || '<subject>'
        /* $lab:coverage:on$ */
    },
