...
// [## Configure]
// [* User Configuration eg. '{"user":"api_authenticator","password":"${POSTGRES_API_PASSWORD}"}']
const user_config = JSON.parse(process.env.API_DB_USER_CONFIG);
//console.log('user_config',user_config)
// [* Database Configuration eg. '{"host":"db","port":5432,"database":"pg_db"}']
const database_config = JSON.parse(process.env.API_DB_CONFIG);
...
const registrations = [
  ...
  // [Register Postgres Connection Pool Plugin]
  {
    ...
    options: {
      config: {
        /* $lab:coverage:off$ */
        user: user_config.user || '<undefined-username>',
        password: user_config.password || '<undefined-database-password>',
        host: database_config.host || '<undefined-host db>',
        database: database_config.database || '<undefined-database>',
        port: database_config.port || '<undefined-port>',
...
//console.log('process.env.API_JWT_SECRET',process.env.API_JWT_SECRET);
...
exports.init = async () => {
  console.log('[* Initialize server for testing]');
  // [* Initialize server for testing]
    try {
      ...
      */
      ...
      console.log('Initialize server for testing ok');
      return server;
    }
};
...
exports.start = async () => {
  ...
  // [* Start server for general use]
    console.log('[* Start server for general use] 1');
    await server.register(
      registrations
    );
